...
// World
	var world = [
		...
		{name: "mechanism",
			...
			directions: ["S"],
			item: [{name: "", idesc: ""}],
			action: [{varindex: 0, item:"Rusty Knife", adesc: "You saw on the rope with the rusty knife. After a few seconds the threads begin to give and with a loud crack the rope whips away from you. The gate crashes into the water, then sinks a few more inches into the mud. The rope dangles from its pulley well out of reach. Even louder than the crash of the barrier, is the silence that follows as the falls trickle into silence"}],
			elev: [{"":""}],
			effects: [{status:waterfall,
				efalse:"You stand at the edge of a mostly-empty basin. Where the river leaves the basin, a huge, ramshackle gate is suspnded over the river by a rope. The rope is firmly attached to a nearby tree, its knots drawn tight be the wieght of the mechanism.",
...
// Button Functions
	...
	function use(){
		debugger;
		console.log(this);
		var itemcheck = this.innerHTML;
		if(itemcheck === current.action[0].item){
			console.log("useable")
			var newtext = $("<p style=display:none>");
			newtext
				.text(current.action.adesc)
				.appendTo(".textarea").fadeIn(3000)
			$(".textarea").animate({scrollTop: 9000}, 3000);
