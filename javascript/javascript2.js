...
// Text-Printing Functions
	// fullprint() subfunctions
		...
		function checklocation () {
			$.map(world, (findloc) => {
				debugger;
				var currentLocCheck = JSON.stringify(player.currentloc);
				var currentAreaCheck = JSON.stringify(findloc.location);
				if(currentLocCheck === currentAreaCheck){
					current = findloc;
					return;
				}
			})
...
// Button Functions
	...
	function grab(){
		if(current.item[0].name !== ""){
			...
			var locstring = JSON.stringify(player.currentloc)
			world[locstring].item = [{name: "", idesc: ""}];
